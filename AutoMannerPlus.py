...
''' A class for classifying gestures as mannerism or meaningful
 Please supply a loaded w2v object to 
...
class AutoMannerPlus(object):
    ...
    # word%_inst, filler%_inst, pause%_inst 
    def extractfeaturesfast(self):
        ...
        for i,j in self.selected.keys():
            # length of instance
            instLen = float(self.patterns[id(i,j)][2] - self.patterns[id(i,j)][1])
            ...
            featurelist[(i,j)]=[np.mean(wrdTime)if wrdTime else 0.0, # Average time to speak word
...
# In this approach, we just extract a number of features and then check their corrcoef (cc)
# with the ground truth. If the cc is high enough, we can build a predictor
def secondapproach():
    alignpath = '/Users/itanveer/Data/ROCSpeak_BL/features/alignments/'
    timepath = '/Users/itanveer/Data/ROCSpeak_BL/Original_Data/Results/'
    gtfile = '/Users/itanveer/Data/ROCSpeak_BL/Ground-Truth/participants_ratings.csv'
